...
parser.add_argument('--magnification', type=int, default=80, help='max magnification of the scanner: 40x, 80x, ...')
parser.add_argument('--zoom_scale', type=float, default=0.25, help='zoom_scale: 0-1')
    ...
    def run(self):
        ...
        while True:
            ...
            if q.empty() and not event.is_set():
                ...
                try:
                    img_dict = q.get(block=event.is_set(), timeout=10)  # Read the first item of the queue
            ...
            for i in range(int(np.floor((img_x - x_size) / (x_size - x_overlap) + 1))):
                for j in range(int(np.floor((img_y - y_size) / (y_size - y_overlap) + 1))):
                    ...
                    if np.sum(mask == 0) / mask.size < args.blank_TH:
                        ...
                        x_offset = int(x_size / pow(4, args.WSI_level))
                        y_offset = int(y_size / pow(4, args.WSI_level))
                        img = slide.read_region((x_start, y_start), args.WSI_level, (x_offset, y_offset)).convert('RGB')

                        if rgb_filter(img):
                            save_path = os.path.join(file_path, str(i) + '_' + str(j) + '.png')
